...
def get_r1(word):
    ...
    # normal form
    match = re.compile(r"[^aeiouy]*[aeiouy]+[^aeiouy](\w*)").match(word)
    if match:
        return match.start(1)
    return len(word)
 
def get_r2(word):
    match = re.compile(r"[^aeiouy]*[aeiouy]+[^aeiouy](\w*)").match(word, get_r1(word))
    if match:
        return match.start(1)
    return len(word)
...
def ends_with_short_syllable(word):
    if len(word) == 2:
        if re.compile(r"^[aeiouy][^aeiouy]$").match(word):
            return True
    if re.compile(r".*[^aeiouy][aeiouy][^aeiouywxY]$").match(word):
        return True
    return False
...
def capitalize_consonant_ys(word):
    if word.startswith('y'):
        word = 'Y' + word[1:]
    return re.compile(r"([aeiouy])y").sub('\g<1>Y', word)
...
def step_1a(word):
    ...
    if word.endswith('s'):
        preceding = word[:-1]
        if re.compile(r"[aeiouy].").search(preceding):
            return preceding
        return word
    return word
